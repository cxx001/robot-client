...
var CardData = [
	...
	0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,				//红桃 A - K
	0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,				//梅花 A - K
	0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,					//方块 A - K
];
...
exp.RandCardList = function ()
{
	...
	for (let i = 0; i < 2; i++) {
		let vecCardData = cbCardData.slice(0);
		cbCardData = [];
		while (vecCardData.length > 0) {
			let index = Math.floor(Math.random() * 10000) % vecCardData.length;
...
exp.AISearchOutCard = function (cbCardData, bCardCount, turnCardData, turnCardCount, bNextWarn) {
	bNextWarn = bNextWarn || false;
	this.SortCardList(cbCardData, bCardCount);
	if (turnCardCount==0)
	{
		let OutCard = {
			bCardCount: 0,		//出牌数目
		...
		let AnalyseResult = this.AnalysebCardData(cbCardData,bCardCount);
		
		//两手出牌，有 2 先出 2
		if (this.GetCardLogicValue(cbCardData[0])==15
			&&this.GetCardType(cbCardData,bCardCount)==exp.CardType.CT_ERROR
			&&bCardCount>1)
		{
			let cbLeftCardData = cbCardData.slice(1, cbCardData.length);
			if (this.GetCardType(cbLeftCardData,bCardCount-1)!=exp.CardType.CT_ERROR)
			{
				OutCard.bCardData[0]=cbCardData[0];
				OutCard.bCardCount = 1;
				IScanOut =  true;
			}
		}
		//最后三张牌，带2，所以出中间一个牌
		if (this.GetCardLogicValue(cbCardData[0])==15 && bCardCount==3 && bNextWarn == false)
		{
			OutCard.bCardData[0]=cbCardData[1];
			OutCard.bCardCount = 1;
			IScanOut =  true;
		}
		if(IScanOut==false)  //********单顺****************
		{
			...
			//搜索连牌
			for (let i=bCardCount-1;i>=5;i--)
			{
				//获取数值
				let cbHandLogicValue=this.GetCardLogicValue(cbCardData[i]);
				//构造判断
				...
				for (let j=i;j>=0;j--)
				{
					if ((this.GetCardLogicValue(cbCardData[j])-cbLineCount) ==cbHandLogicValue
						&&bombVlue!=this.GetCardLogicValue(cbCardData[j])
						&&this.GetCardLogicValue(cbCardData[j])<15) //不能拆炸弹
					{
						//增加连数
						SigneHand[cbLineCount++]=cbCardData[j];
					}
				}
		...
		///// 333 带 2
		if(AnalyseResult.cbThreeCount>0&&bCardCount>=5&&IScanOut==false)
		{
			if(this.GetCardLogicValue(AnalyseResult.cbThreeCardData[AnalyseResult.cbThreeCount*3-1])==3)
			{
				...
				{
					let pos = bCardCount-5;
					OutCard.bCardData = cbCardData.slice(pos, pos + 5);
				}
		...
		//最后一把
		if(AnalyseResult.cbThreeCount==1&&bCardCount<=4&&IScanOut==false)
		{
			OutCard.bCardData = cbCardData.slice(0);
		...
		if (IScanOut==false) //三带二
		{
			...
			TempTurnCard[4] =0x05;
			TempTurnCount = 5;
			if (this.SearchOutCard(cbCardData,bCardCount,TempTurnCard,TempTurnCount,OutCardResult)==true)
			{
				if (OutCardResult.cbCardCount==5)
				{
					//开始打JJJ以上 带二 不允许
					if(this.GetCardLogicValue(OutCardResult.cbResultCard[0])>11&&bCardCount>10)
					{
						IScanOut = false;
					}
					else //if(this.GetCardLogicValue(OutCardResult.cbResultCard[0])!=bombVlue)
					{
						//炸弹不能拆
						let isBomb = false;
						let outvalue = this.GetCardLogicValue(OutCardResult.cbResultCard[0]);
						for (let i = 0; i < AnalyseResult.cbFourCount; i++) {
							let value = this.GetCardLogicValue(AnalyseResult.cbFourCardData[i*4]);
							if (outvalue == value) {
								isBomb = true;
							}
						}
		...
		{
			let FirstLogV;
			FirstLogV = this.GetCardLogicValue(cbCardData[bCardCount-1]);
			wSameCardNum = 0;
			for(let i=0;i<bCardCount;i++)
			{
				if (FirstLogV== this.GetCardLogicValue(cbCardData[bCardCount-1-i]))
					OutCard.bCardData[wSameCardNum++]= cbCardData[bCardCount-1-i];
				else break;
			}
			if (wSameCardNum==3)
			{
		...
		//一次出完 
		if (this.GetCardType(cbCardData,bCardCount)!=exp.CardType.CT_ERROR)
		{
			if (bCardCount<=6&&AnalyseResult.cbFourCount>0)
			{
	...
	else
	{
		...
		if (this.SearchOutCard(cbCardData,bCardCount,turnCardData,turnCardCount,OutCardResult)==true)
		{
			let OutCard = {
				bCardCount: 0,				//出牌数目
				bCardData: [],				//扑克列表
			...
			{    //****************如果把炸弹拆了，强制出炸弹***********************
				...
				{
					let isBomb = false;
					let outvalue = this.GetCardLogicValue(OutCardResult.cbResultCard[i]);
					for (let i = 0; i < AnalyseResult.cbFourCount; i++) {
						let value = this.GetCardLogicValue(AnalyseResult.cbFourCardData[i*4]);
						if (outvalue == value) {
							isBomb = true;
			...
			{
				OutCard.bCardData[0] = this.GetHandMaxCard(cbCardData,bCardCount);
				OutCard.bCardCount = 1;
			}
			else if (OutCard.bCardCount==1&&bCardCount>1)
			{
				this.SortCardList(cbCardData,bCardCount);
				if (this.GetCardLogicValue(cbCardData[1])>this.GetCardLogicValue(turnCardData[0]))
				{
					OutCard.bCardData[0] = cbCardData[1];
					OutCard.bCardCount = 1;
				}
...
exp.GetChairIDByUid = function(uid, players) {
	...
	console.error('GetChairIDByUid error.!', uid, players);
};